local library = loadstring(game:HttpGet("https://pastebin.com/raw/GAGA5H8t"))();

local Gamer = library:CreateSection("Blox Fruits");
Gamer:Label("~ Press Right Shift To Hide GUI ~");
Gamer:Button("Mhee Hub", function()
loadstring(game:HttpGet("https://raw.githubusercontent.com/BunnySalf/Hentai/main/MHEEHUB",true))()
end)
Gamer:Button("(OP) Xenos Hub", function()
loadstring(game:HttpGet("https://reaperking.xyz/bloxfruit", true))()
end)
Gamer:Button("Evil Hub", function()
loadstring(game:HttpGet("https://raw.githubusercontent.com/Natthawat69/EvilHub-FREE/main/Script.lua"))()
end)
Gamer:Button("BIG GUI", function()
			loadstring(game:HttpGet("https://raw.githubusercontent.com/noob1ee1/blox-fruits-script/main/bloxfruits"))()
end)
Gamer:Button("WalterHub GUI GUI", function()
			loadstring(game:HttpGet("https://raw.githubusercontent.com/WalterCums/hub/main/Detection.lua", true))()
end)
Gamer:Button("BK Hax", function()
loadstring(game:HttpGet(("https://raw.githubusercontent.com/koonpeatch/PeatEX/master/BKHAX/BloxFruits"),true))() 
end)
Gamer:Label("~ Thanks for using Furk Ultra ~");

library:Ready();
